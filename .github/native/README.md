...
2. Users and product release jobs run `fetch.sh`. It downloads the matching platform archive once, verifies it, and installs it below `.cache/record-native` before the Go/CGO link step. Pull-request CI falls back to the same pinned source builders when a new versioned SDK release has not been published yet.
