...
func TestCommandsReachTheHookRegistryTheProfilePublishes(t *testing.T) {
	entries, err := harness.BaseExtensions(harness.BaseConfig{Directory: t.TempDir()})
	if err != nil {
		t.Fatal(err)
	}
	...
	command := coretool.Command{
		Name: "probe",
		Run:  func(context.Context, *coretool.Execution) (any, error) { return "ok", nil },
	}
	var registry *hooks.Registry
	var executor coretool.CommandExecutor
	...
	if _, err := executor.Execute(t.Context(), "probe", &coretool.Execution{ID: "probe-1"}); err != nil {
		t.Fatal(err)
	}
