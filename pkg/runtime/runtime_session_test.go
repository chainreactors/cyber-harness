...
func TestResolveJSONLRecordPathSemantics(t *testing.T) {
	option := &cfg.Option{}
	if got := ResolveJSONLRecordPath(option); got != "" {
		t.Fatalf("one-shot path = %q, want empty", got)
	}

	option.SaveSession = true
	if got := ResolveJSONLRecordPath(option); !strings.HasSuffix(got, ".jsonl") || !strings.Contains(got, "sessions") {
	...
	option.SaveSession = true
	if got := ResolveJSONLRecordPath(option); got != "old.jsonl" {
