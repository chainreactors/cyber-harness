...
func newTestConsole(t *testing.T, option *cfg.Option, provider agent.Provider, stdout, stderr io.Writer) *AgentConsole {
	...
	session, err := rt.OpenSession(context.Background(), runtimepkg.SessionOptions{ID: "console-test"})
	if err != nil {
		t.Fatal(err)
	}
...
func TestConsoleSubmissionsUseRuntimeFIFOAndLimit(t *testing.T) {
	...
	for i := 1; i < runtimepkg.DefaultSessionPendingLimit; i++ {
	...
	close(p.release)
	c.work.Wait()
	if p.calls.Load() != runtimepkg.DefaultSessionPendingLimit {
...
func TestConsoleStopCancelsOnlyItsOwnSubmissions(t *testing.T) {
	...
	other, err := c.session.Run(context.Background(), runtimepkg.RunInput{Content: []*aop.Content{aop.Text("inline")}})
	if err != nil {
		t.Fatal(err)
	}
