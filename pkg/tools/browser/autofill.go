...
func (c *Command) execAutofill(ctx context.Context, args []string) (string, error) {
	...
	return sess.withPage(ctx, func(page *rod.Page) (string, error) {
		...
		if err != nil {
			return "", fmt.Errorf("browser autofill: failed to get forms: %w", err)
		}
		if formsRes.Value.Nil() || formsRes.Value.Str() == "[]" {
		...
		if formIndex < 0 || formIndex >= len(forms) {
			return "", fmt.Errorf("browser autofill: form index %d out of range (found %d forms)", formIndex, len(forms))
		}
...
func parseAutofillOpts(args []string) (sessName string, formIndex int, overrides map[string]string, err error) {
	...
	if len(args) == 0 {
		return "", 0, nil, fmt.Errorf("browser autofill: session name required")
	}
	sessName = args[0]
	...
	for i := 1; i < len(args); i++ {
		...
		case "--form":
			if i+1 >= len(args) {
				return "", 0, nil, fmt.Errorf("browser autofill: --form requires an index")
			}
			i++
			idx, parseErr := strconv.Atoi(args[i])
			if parseErr != nil {
				return "", 0, nil, fmt.Errorf("browser autofill: --form must be an integer: %w", parseErr)
			}
			if idx < 0 {
				return "", 0, nil, fmt.Errorf("browser autofill: --form must be >= 0")
			}
			formIndex = idx
		case "--data":
			if i+1 >= len(args) {
				return "", 0, nil, fmt.Errorf("browser autofill: --data requires key=value pairs")
			}
			i++
			...
			for _, pair := range pairs {
				...
				if len(kv) != 2 || strings.TrimSpace(kv[0]) == "" {
					return "", 0, nil, fmt.Errorf("browser autofill: invalid --data pair %q (expected key=value)", pair)
				}
				overrides[strings.TrimSpace(kv[0])] = kv[1]
			}
		default:
			if strings.HasPrefix(args[i], "-") {
				return "", 0, nil, fmt.Errorf("browser autofill: unknown flag: %s", args[i])
			}
			return "", 0, nil, fmt.Errorf("browser autofill: unexpected argument: %s", args[i])
		}
	}
	return
}
