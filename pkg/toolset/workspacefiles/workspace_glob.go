...
type workspaceGlobTool struct {
	workDir  string
	globbers []VirtualGlobber
}

func newWorkspaceGlobTool(workDir string, globbers ...VirtualGlobber) *workspaceGlobTool {
	return &workspaceGlobTool{workDir: workDir, globbers: globbers}
}

func (t *workspaceGlobTool) Name() string { return "glob" }

func (t *workspaceGlobTool) Description() string {
...
func (t *workspaceGlobTool) Definition() *coretool.Definition {
	return coretool.Def("glob", t.Description(), GlobArgs{})
}

func (t *workspaceGlobTool) Execute(ctx context.Context, arguments string) (*coretool.Result, error) {
	effective := *t
