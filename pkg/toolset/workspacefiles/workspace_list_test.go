...
func TestWorkspaceListToolReturnsStructuredDirectoryEntries(t *testing.T) {
	dir := t.TempDir()
	...
	result, err := newWorkspaceListTool(dir).Execute(context.Background(), `{}`)
	if err != nil {
		t.Fatal(err)
	}
...
func TestWorkspaceListToolUsesInvocationWorkdir(t *testing.T) {
	...
	result, err := newWorkspaceListTool(defaultDir).Execute(ctx, `{"path":"."}`)
	if err != nil {
		t.Fatal(err)
	}
	var listing ListResult
