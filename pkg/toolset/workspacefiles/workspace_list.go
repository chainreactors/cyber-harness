...
// workspaceListTool lists directory entries through the host filesystem API.
type workspaceListTool struct {
	workDir string
}

func newWorkspaceListTool(workDir string) *workspaceListTool {
	return &workspaceListTool{workDir: workDir}
}

func (t *workspaceListTool) Name() string { return "ls" }
...
func (t *workspaceListTool) Definition() *coretool.Definition {
	return coretool.Def("ls", t.Description(), ListArgs{})
}

func (t *workspaceListTool) Execute(ctx context.Context, arguments string) (*coretool.Result, error) {
