...
`AgentRuntime.RegisterNamespaces(mux)`；嵌入者也可直接调用 `mux.Register`。
...
验证见 [process_test.go](process_test.go)。这两个示例均不调用模型或工具。
...
| `aop.NamespaceMux` | 现有协议路由表；由业务拥有者注册 |
...
- `Close` 停止后续请求与发送，取消 Host context，等待已进入的分发和写入。
  它可重复调用；它不关闭借用的 Runtime、mux、输入输出流。
...
已包括公开 inline 接入、产品 stdio 接线、真实子进程通信、异步响应、连接取消和
关闭隔离、并发发送与错误保留、Node 具体运行时接线回归，以及协议依赖边界检查。
`queuedEnvelopeStream`、嵌套 Host、可选控制接口和 Stdio 重复状态均不保留。

Session/App/Console 的进一步包迁移属于后续运行层重构；它们不是通信 Host 的职责。
产品 Runtime 位于 `pkg/runtime`，Console 位于 `pkg/console`；Host 不持有二者。
