	...
	for turn = 1; ; turn++ {
		...
		if err != nil {
			...
			if isContextOverflowError(err) && !overflowRecoveryAttempted {
				compacted, compactErr := runAutoCompaction(ctx, cfg, em, transcript, "overflow")
				if compactErr != nil {
...
func runAutoCompaction(ctx context.Context, cfg Config, em *aopEmitter, transcript *transcript, reason string) (bool, error) {
