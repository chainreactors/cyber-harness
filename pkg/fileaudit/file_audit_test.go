...
type collector struct {
	...
	detach   *eventbus.Subscription[*filepb.Access]
}
...
func collect(t *testing.T, audit *Audit) *collector {
	t.Helper()
	c := &collector{}
	...
	t.Cleanup(func() { c.detach.Cancel() })
	return c
}
...
func TestSnapshotSkipsIgnoredDirectories(t *testing.T) {
	...
	snapshot, err := TakeSnapshot(dir, Options{Enabled: true})
	if err != nil {
		t.Fatalf("TakeSnapshot: %v", err)
	}
	if len(snapshot) != 1 {
...
func TestSnapshotRefusesOversizedTree(t *testing.T) {
	...
	if _, err := TakeSnapshot(dir, Options{Enabled: true, MaxEntries: 3}); err == nil {
...
func TestWriteToolRecordsCreateWriteAndEdit(t *testing.T) {
	...
	dir := t.TempDir()
	tool := workspaceTools(t, dir, nil, audit)
	path := filepath.Join(dir, "note.txt")
	ctx := context.Background()

	if _, err := tool.ExecuteTool(ctx, "write", `{"path": "note.txt", "content": "first\n"}`); err != nil {
		t.Fatalf("write: %v", err)
	}
	if _, err := tool.ExecuteTool(ctx, "write", `{"path": "note.txt", "content": "second\n"}`); err != nil {
		t.Fatalf("rewrite: %v", err)
	}
	if _, err := tool.ExecuteTool(ctx, "write", `{"path": "note.txt", "edits": [{"old_text": "second", "new_text": "third"}]}`); err != nil {
		t.Fatalf("edit: %v", err)
	}
...
func TestReadToolRecordsWhatTheModelReceived(t *testing.T) {
	...
	tool := workspaceTools(t, dir, nil, audit)
	if _, err := tool.ExecuteTool(context.Background(), "read", `{"path": "data.txt"}`); err != nil {
		t.Fatalf("read: %v", err)
	}
...
func TestReadToolDoesNotRecordVirtualReads(t *testing.T) {
	...
	tool := workspaceTools(t, t.TempDir(), staticVirtualReader{"aiscan://skill": "content"}, audit)
	if _, err := tool.ExecuteTool(context.Background(), "read", `{"path": "aiscan://skill"}`); err != nil {
		t.Fatalf("read: %v", err)
	}
