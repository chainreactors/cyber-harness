...
func TestCatalogBindsTypedContributions(t *testing.T) {
	catalog := New()
	...
	if _, err := catalog.Add(bindings...); err != nil {
		t.Fatal(err)
	}

	mux := aop.NewNamespaceMux(t.Context())
	if err := catalog.Bind(mux); err != nil {
		t.Fatal(err)
	}
...
func TestCatalogRejectsDuplicateNamespace(t *testing.T) {
	catalog := New()
	if _, err := catalog.Add(binding(&filepb.ProtocolMessage{}, noOpHandler)); err != nil {
		t.Fatal(err)
	}
	if _, err := catalog.Add(binding(&filepb.ProtocolMessage{}, noOpHandler)); err == nil {
...
func TestCatalogHandleRemovalAffectsFutureSnapshots(t *testing.T) {
	catalog := New()
	handle, err := catalog.Add(binding(&filepb.ProtocolMessage{}, noOpHandler))
	if err != nil {
		t.Fatal(err)
	}

	existing := aop.NewNamespaceMux(t.Context())
	if err := catalog.Bind(existing); err != nil {
		t.Fatal(err)
	}
	...
	if err := catalog.Bind(future); err != nil {
		t.Fatal(err)
	}
