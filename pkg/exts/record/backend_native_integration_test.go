//go:build record && record_integration && cgo && (windows || linux)
...
func TestNativeDesktopScreenshotAndRecord(t *testing.T) {
	...
	defer backend.Close()
	target, err := backend.Resolve(t.Context(), recordtool.CaptureRequest{Target: "desktop", FPS: 10})
	if err != nil {
		t.Fatal(err)
	}
	img, err := backend.Screenshot(t.Context(), target)
	if err != nil {
		t.Fatal(err)
	}
