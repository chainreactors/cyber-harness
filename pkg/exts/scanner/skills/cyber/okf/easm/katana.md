...
Cyber resolves a browser in this order:
...
3. `CYBER_BROWSER_PATH` shared by Cyber Playwright, nuclei headless replay, and Katana.
...
Automatic reuse means the executable is shared while each engine starts an isolated process/profile. Cyber does not automatically attach to a user's running browser. Use `--chrome-ws-url` only when process-level reuse is intentional.
