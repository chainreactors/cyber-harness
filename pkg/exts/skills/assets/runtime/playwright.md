...
Record successful browser commands as a nuclei-shaped headless YAML template. This is cyber's codegen workflow: it records CLI operations after they succeed and emits declarative browser actions instead of Node.js test code.
...
The generated YAML uses the nuclei headless schema. Templates containing only the upstream core actions remain portable to compatible nuclei/neutron runners. Actions marked as Cyber extensions require `playwright template` in Cyber; upstream nuclei does not know those action names.
...
playwright fill s1 "input[name=q]" "cyber_canary_8f2a"
...
| `wait-for-url/request/response` | `waiturl` / `waitrequest` / `waitresponse` (Cyber) |
...
| `is-visible/hidden/checked/enabled/disabled` | `assert` preserving the observed boolean state (Cyber) |
| `screenshot` | `screenshot`, including `--selector` |
| `set-extra-headers` | `setheader` (one per header) |
| `hover`, `dblclick`, `focus`, `blur` | same-named Cyber action |
| `check`, `uncheck` | idempotent same-named Cyber action |
| `dispatch-event` | `dispatch` (Cyber) |
| `set-viewport` | `setviewport` (Cyber) |
| `reload`, `go-back`, `go-forward` | `reload`, `goback`, `goforward` (Cyber) |
| `set-content` | `setcontent` (Cyber) |
| local/session storage set/delete/clear | `storage` (Cyber) |
| local/session storage get/list | `extract` with `target: storage` |
| cookie set/delete/clear | `cookie` (Cyber) |
...
Templates support the 29-action nuclei-compatible core plus the Cyber extensions above, DSL expressions (`{{rand_int()}}`, `{{replace()}}`, etc.), payload iteration (sniper/pitchfork/clusterbomb), template variables, matchers, and extractors.
...
| microsoft/playwright-cli | cyber playwright | notes |
|---|---|---|
| `open` | `open` | |
| `goto` | `goto` | |
| `close` | `close` | |
...
## cyber Extensions (no playwright-cli equivalent)
...
- Browser discovery uses `CYBER_BROWSER_PATH` first, then installed Chrome, Chromium, or Edge in the system PATH and standard OS install locations. Rod's cached/downloaded Chromium is used only when neither is available. Katana uses the same discovery policy.
- System-browser reuse means reusing the executable while Cyber launches an isolated managed process/profile. To control an already running browser process, use `attach --cdp <url>` or `open --cdp <url>`.
