export default {
  openSettings: 'Open settings',
  agentsConnected: '{{count}} agent(s) connected',
  noAgents: 'No agents connected',
  llmReady: 'LLM Ready',
  llmOffline: 'LLM Offline',
  closePanel: 'Close panel',
  switchLLMProfile: 'Switch LLM profile',
  confirmTitle: 'Please confirm',
  confirm: 'Confirm',
  cancel: 'Cancel',
  closeDialog: 'Close dialog',
  switchToEnglish: 'Switch to English',
  switchToChinese: 'Switch to Chinese',
  crashTitle: 'Something went wrong',
  crashHint: 'The interface hit an unexpected error. Reloading usually clears it.',
  crashReset: 'Try again',
  crashReload: 'Reload',
  projectTitle: 'Projects',
  projectSwitch: 'Switch project',
  projectDefault: 'Default',
  projectNewPlaceholder: 'New project name',
  projectCreate: 'Create project',
  projectCreateFailed: 'Could not create project',
  projectDelete: 'Delete project',
  projectDeleteConfirm: 'Confirm delete? Also removes the {{n}} assets in this project',
  projectDeleteCancel: 'Cancel',
  projectDeleteFailed: 'Could not delete project',
  // Quick connect
  quickConnect: 'Quick connect an agent',
  quickConnectTitle: 'Download & connect an agent',
  quickConnectInstall: 'Install + Connect',
  quickConnectOnly: 'Connect Only',
  quickConnectCopy: 'Copy command',
  quickConnectDownloadSource: 'Download source',
  quickConnectGlobal: 'Global',
  quickConnectChina: 'China',
  quickConnectHint: 'The token is configured automatically; replace only NODE_NAME with a unique node name before running.',
  quickConnectTokenReady: 'Token configured',
  quickConnectTokenLoading: 'Securely configuring token…',
  quickConnectTokenError: 'Could not get the agent connection token',
  quickConnectRetry: 'Retry',
  // Header LLM health indicator
  llmUnconfigured: 'LLM not configured',
  llmChecking: 'Checking…',
  llmUnreachable: 'LLM unreachable',
  llmHealthSettings: 'Click to open settings',
  // Mobile session drawer (opened from the header menu button)
  openSessions: 'Chat history',
  authChecking: 'Checking session…',
  loginTitle: 'Access Cyber',
  loginDescription: 'Enter the access token generated at startup or configured on the command line.',
  loginTokenLabel: 'Access token',
  loginTokenPlaceholder: 'Enter token',
  loginTokenRequired: 'Enter an access token',
  loginShowToken: 'Show token',
  loginHideToken: 'Hide token',
  loginSecurityHint: 'The login session is stored in an HttpOnly cookie; the token is retrieved only when generating an agent command.',
  loginSubmit: 'Sign in',
  loginSubmitting: 'Verifying…',
  loginInvalid: 'The token is invalid. Check it and try again.',
  loginUnavailable: 'The server could not be reached. Try again shortly.',
  logout: 'Sign out',
}
