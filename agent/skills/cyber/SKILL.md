---
name: cyber
description: Use this skill for Cyber's attack surface management and penetration-testing capabilities, including scanner pseudo-commands, supporting security tools, vulnerability verification, evidence handling, and assessment reporting.
---
...
- `read` / `write` / `glob`: workspace file operations. `read` also loads embedded skill files via `cyber://` URIs.
...
The pseudo-commands and utilities below provide Cyber's dedicated ASM and penetration-testing toolset. They run through `bash` and are available only when exposed by the current runtime.
...
When producing a scan report, follow the format and verification semantics in `cyber://skills/cyber/reference/report.md`. Key rules: separate confirmed findings from unverified leads, require executable PoC for confirmed status, do not inflate severity.
