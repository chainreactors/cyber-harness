...
func TestStandaloneManagerAndLiveCommandRegistration(t *testing.T) {
	...
	if err := m.Start(t.Context(), t.Context()); err != nil {
		t.Fatal(err)
	}
	t.Cleanup(func() {
	...
	if len(m.CommandSpecs(false)) != before {
		t.Fatal("partial command publication")
	}
	if _, ok := m.lookupCommand("/new"); ok {
		t.Fatal("conflicting batch leaked")
	}
	if err := m.Close(t.Context()); err != nil {
		t.Fatal(err)
	}
	if err := m.RegisterCommand(Command{Spec: &types.CommandSpec{Name: "/late"}, Handler: func(context.Context, *Session, []string) (*types.CommandResult, error) { return nil, nil }}); err == nil {
...
func TestConcurrentCommandRegistrationAndDiscovery(t *testing.T) {
	...
	for i := 0; i < 24; i++ {
		...
		go func(i int) {
			...
			if err := m.RegisterCommand(Command{Spec: &types.CommandSpec{Name: name}, Handler: func(context.Context, *Session, []string) (*types.CommandResult, error) { return nil, nil }}); err != nil {
				t.Error(err)
			}
			_ = m.CommandSpecs(false)
			if _, ok := m.lookupCommand(name); !ok {
