...
| 版本 | 平台 | 内容 |
| --- | --- | --- |
| `cyber` | Linux/macOS/Windows amd64、arm64 | standard：scan、agent、IOA 和纯 Go 工具集 |
| `cyber-full` | Linux/macOS amd64、arm64；Windows amd64 | standard + Web、Playwright、passive、katana |
...
- IOA 使用 `--ioa-url`；`cyber ioa --server-url` 不再接受。
...
PTY/terminal 路由位于 `pkg/terminal`。`core` 仅保留 Cyber 的领域配置、能力规划、事件、资源和遥测等内部基础设施；可复用的终端传输不属于 core。
