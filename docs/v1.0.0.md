...
正式发布前需通过：Go 单元/竞态/架构测试、`go vet`、`golangci-lint`、protobuf 与资源生成一致性、standard/full/runner 构建、Web 前端构建与 E2E、cyber-ui viewer 测试、Windows 产物启动验证。CI 通过 release workflow 的 dry-run 模式执行同一套构建、打包和 smoke test，但不会创建 Git tag 或 Release。依赖漏洞报告单独跟踪，不作为本次 v1.0.0 发布门禁。
