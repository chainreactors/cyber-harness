...
Agent 可能看到 `cyber://skills/...` 这样的虚拟路径。这些资源由运行时从内嵌知识库读取，不是 HTTP 下载地址。使用者通常不需要手工解压这些文件。
...
同名 Skill 按来源优先级选择：内置资源、扩展 Bundle、项目的 `.cyber/skills/`、`.agent/skills/`，最后是 CLI 指定路径。后面的来源覆盖前面的同名内容，使项目可以定制默认知识。
