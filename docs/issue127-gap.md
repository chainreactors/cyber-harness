...
核对日期：2026-09-12。Issue 127 尚未完成；当前工作区仍有大量既有迁移变更。
...
| Registry lease | 未完成；已有 Registrar 的 Register/UnregisterOwner 不等于 lease 已实现 |
...
文件/工具注册表测试及 App 等路径仍有直接把 context.Context 传给 Extension.Load 的旧调用。后续必须按真实 Set 所有权迁移 Load/Close 调用，不能引入双生命周期接口来绕过错误。当前没有全仓测试、full 标签、两个产品入口构建或外部适配器通过的结论。
