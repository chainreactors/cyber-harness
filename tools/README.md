...
Agent 工具实现 `core/tool.Tool`；Bash 原生命令使用 `pkg/tool.Command`。二者是不同
调用协议，不应通过适配器伪装成一个万能抽象。

Tool Registry 和 Command Registry 分别定义 `tool.Tool` 与 `tool.Command` typed Point。插件在 Extension.Load 中调用
