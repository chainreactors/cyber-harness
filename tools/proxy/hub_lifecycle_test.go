...
func TestHubShutdownReportsStorageFailureOnce(t *testing.T) {
	for _, throughInfra := range []bool{false, true} {
		...
		t.Run(name, func(t *testing.T) {
			...
			// A real closed metadata index makes the final Sync/Close fail.
			...
			if store.indexFile != nil || store.indexSub != nil {
				t.Fatal("completed cleanup retained metadata index ownership")
			}
...
func TestHubShutdownTimeoutRetainsDrainAndFinalError(t *testing.T) {
	...
	synctest.Test(t, func(t *testing.T) {
		...
		want := errors.New("metadata index callback failed")
		var err error
		...
		if _, err := file.Stat(); err != nil {
			t.Fatalf("metadata index closed before callback finished: %v", err)
		}
		unblock()
		...
		if _, err := file.WriteString("after shutdown"); !errors.Is(err, os.ErrClosed) {
			t.Fatalf("metadata index remains open: %v", err)
		}
