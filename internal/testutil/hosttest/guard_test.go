...
const (
	hosttestPath = "github.com/chainreactors/cyber/internal/testutil/hosttest"
	apptestPath  = "github.com/chainreactors/cyber/internal/testutil/apptest"
)
...
func TestOnlyTestsLinkTheHostHelpers(t *testing.T) {
	...
	err := filepath.WalkDir(root, func(path string, entry fs.DirEntry, err error) error {
		...
		for _, imported := range file.Imports {
			...
			if value == hosttestPath && filepath.ToSlash(filepath.Dir(relative)) == "internal/testutil/apptest" {
				continue
			}
