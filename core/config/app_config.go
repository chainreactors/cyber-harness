...
func AppConfig(option *Option, features RuntimeFeatures, logger telemetry.Logger) RuntimeConfig {
	return RuntimeConfig{
		...
		Scanner: ScannerConfig{
			...
			VerifyMode:   ResolveString(option.ScanConfig.Verify, DefaultVerify),
			Proxy:        option.Proxy,
		...
		Tools: ToolConfig{
			...
			BashTimeout:   300,
			TavilyKeys:    resolveTavilyKeys(option.TavilyKey, ResolveString(option.SearchConfig.TavilyKeys, DefaultTavilyKeys)),
