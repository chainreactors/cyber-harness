...
func TestRegistryPreservesCommandArguments(t *testing.T) {
	// Even a command named scan owns its flag policy; registration must not
	// inject scanner-specific flags into another host's command.
	for _, name := range []string{"echo", "scan"} {
		t.Run(name, func(t *testing.T) {
			...
			tokens := []string{name, "--mode", "quick"}
			if _, err := registry.Run(t.Context(), tokens, &Execution{}); err != nil {
				t.Fatal(err)
			}
			if !slices.Equal(got, tokens[1:]) {
				t.Fatalf("arguments = %q, want %q", got, tokens[1:])
			}
		})
	}
}
...
func loadTestRegistry(t *testing.T, batches ...testCommandBatch) (*CommandRegistry, *extension.Set) {
	t.Helper()
	registry := NewCommandRegistry()
