...
type registryTool struct {
	def *coretool.Definition
	run func(context.Context, string) (*coretool.Result, error)
}

func echoTool(name string) *registryTool {
	return &registryTool{def: coretool.Def(name, "echo", struct{}{})}
}
func (t *registryTool) Name() string                     { return t.def.Name }
func (t *registryTool) Description() string              { return t.def.Description }
func (t *registryTool) Definition() *coretool.Definition { return t.def }
func (t *registryTool) Execute(ctx context.Context, args string) (*coretool.Result, error) {
	if t.run != nil {
		return t.run(ctx, args)
	}
	return coretool.TextResult(args), nil
}

type contributor struct {
	registry *coretool.ToolRegistry
	values   []coretool.Tool
...
func registrySet(t *testing.T, entries ...extension.Extension) (*coretool.ToolRegistry, *extension.Set) {
	t.Helper()
...
func TestRegistryPublishesOnlyAfterLoad(t *testing.T) {
	first := echoTool("echo")
	registry, set := registrySet(t, &contributor{values: []coretool.Tool{first}})
	if len(registry.ToolDefinitions()) != 0 {
		t.Fatal("staged definitions were published")
	}
	if _, err := registry.ExecuteTool(t.Context(), "echo", ""); !errors.Is(err, coretool.ErrUnavailable) {
		t.Fatal(err)
	}
...
func TestRegistryUsesSharedHookBoundary(t *testing.T) {
	...
	value := echoTool("echo")
	value.run = func(_ context.Context, args string) (*coretool.Result, error) {
		runs++
		return coretool.TextResult(args), nil
	}
	...
	set, err := extension.New(
		...
		registry,
		&contributor{registry: registry, values: []coretool.Tool{value}},
	)
	if err != nil {
		t.Fatal(err)
	}
...
func TestRegistryRegistrationIsAtomic(t *testing.T) {
	registry := coretool.NewToolRegistry()
	first := &contributor{registry: registry, values: []coretool.Tool{echoTool("echo")}}
	second := &contributor{registry: registry, values: []coretool.Tool{echoTool("new"), echoTool("echo")}}
	set, err := extension.New(
	...
	if err := set.Load(t.Context()); !errors.Is(err, coretool.ErrDuplicate) {
...
func TestRegistryRejectsInvalidBatchWithoutPartialState(t *testing.T) {
	...
	value := &contributor{registry: registry, load: func(scope *extension.Scope) error {
		...
		if _, err := registry.Add(echoTool("duplicate"), echoTool("duplicate")); !errors.Is(err, coretool.ErrDuplicate) {
			t.Fatal(err)
		}
		return extension.Add[coretool.Tool](scope, echoTool("partial"))
	}}
	set, err := extension.New(
		extension.Provided[*hooks.Registry](hooks.New()),
		registry,
...
func TestRegistryDrainProtectsContributorResources(t *testing.T) {
	synctest.Test(t, func(t *testing.T) {
		...
		value.run = func(ctx context.Context, _ string) (*coretool.Result, error) {
			close(started)
			<-ctx.Done()
			close(canceled)
			<-release
		...
		registry, set := registrySet(t, &contributor{
			values: []coretool.Tool{value}, close: func(context.Context) error { closed = true; return nil },
		})
		if err := set.Load(t.Context()); err != nil {
			t.Fatal(err)
		}
...
func TestRegistryPanicReleasesAdmission(t *testing.T) {
	...
	value.run = func(context.Context, string) (*coretool.Result, error) { panic("private data") }
	registry, set := registrySet(t, &contributor{values: []coretool.Tool{value}})
	if err := set.Load(t.Context()); err != nil {
		t.Fatal(err)
	}
